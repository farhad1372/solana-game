    ...
    >
      ...
      <template #footer>
        <div class="con-footer mt-5 d-flex justify-end">
          <vs-button @click="sqlDialog = false" class="px-6"> Exit </vs-button>
        </div>
      </template>
    </vs-dialog>
    <vs-card
      type="0"
      class="w-100 single-chart-card"
      :class="{ 'card-loading': !singleChartData }"
    >
      <template #title>
        <div class="d-flex justify-space-between pt-2" style="min-height: 5rem">
          ...
          <vs-tooltip style="align-self: start">
            ...
            <template #tooltip> SQL Statement </template>
          </vs-tooltip>
        </div>
      </template>
      <template #text>
